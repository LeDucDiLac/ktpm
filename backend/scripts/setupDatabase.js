...
async function fixHouseholdIndex() {
  ...
  try {
    ...
    const householdsExists = collections.some(
      (col) => col.name === "households"
    );
    ...
    if (householdsExists) {
      ...
      const householdCodeIndex = indexes.find(
        (index) =>
...
async function updatePaymentModel() {
  ...
  try {
    ...
    if (collections.length > 0) {
      console.log(
        "🔍 Đang kiểm tra và cập nhật chỉ mục cho collection payments..."
      );
      ...
      await db
        .collection("payments")
        .createIndex({ fee: 1, household: 1, period: 1 }, { unique: true });
      ...
      const result = await db
        .collection("payments")
        .updateMany({ period: { $exists: false } }, [
          {
            $set: {
              period: {
                $cond: {
      ...
      console.log(
        `✅ Đã cập nhật ${result.modifiedCount} bản ghi thanh toán với trường period`
      );
    } else {
  ...
  try {
    ...
    for (let floor = 1; floor <= 5; floor++) {
      for (let unit = 1; unit <= 3; unit++) {
        const apartmentNumber = `${String.fromCharCode(64 + floor)}${unit
          .toString()
          .padStart(2, "0")}`;

        householdsToCreate.push({
          apartmentNumber: apartmentNumber,
          address: `Căn hộ ${apartmentNumber}, Chung cư BlueMoon, Quận ${
            Math.floor(Math.random() * 12) + 1
          }, TP.HCM`,
          note: `Hộ gia đình ${apartmentNumber} - Ngày tạo: ${new Date().toLocaleDateString(
            "vi-VN"
          )}`,
          active: true,
        });
    ...
    for (let monthOffset = 5; monthOffset >= 0; monthOffset--) {
      ...
      for (const household of payingHouseholds) {
        ...
        for (const fee of selectedFees) {
          ...
          if (monthOffset === 0 && paymentMonth.getMonth() === 5) {
            // Tháng 6 (index 5)
            ...
            paymentDate = new Date(
              paymentMonth.getFullYear(),
              paymentMonth.getMonth(),
              Math.floor(Math.random() * 16) + 1 // Ngày 1-16
            );
          }
      ...
      if (monthOffset === 1) {
        // Chỉ tạo nợ cho tháng 5 (tháng trước)
        const nonPayingHouseholds = allHouseholds.filter(
          (h) =>
            !payingHouseholds.some(
              (ph) => ph._id.toString() === h._id.toString()
            )
        );

        // Đảm bảo có ít nhất 20% hộ gia đình có nợ phí quản lý hàng tháng
        const managementFee = fees.find(
          (fee) =>
            fee.name === "Phí quản lý hàng tháng" || fee.feeCode === "PHI001"
        );

        if (managementFee) {
          const overdueHouseholds = nonPayingHouseholds.slice(
            0,
            Math.max(5, Math.floor(allHouseholds.length * 0.2))
          );
          ...
          for (const household of overdueHouseholds) {
            ...
            const otherFees = fees
              .filter((f) => f._id.toString() !== managementFee._id.toString())
              .sort(() => 0.5 - Math.random())
            ...
            for (const fee of otherFees) {
              paymentsToCreate.push({
                ...
                note: `${
                  fee.name
                } quá hạn - Tháng 5/${new Date().getFullYear()}`,
              });
            }
          }
        }
    ...
    const managementFee = fees.find(
      (fee) => fee.name === "Phí quản lý hàng tháng" || fee.feeCode === "PHI001"
    );

    if (managementFee) {
      for (const household of allHouseholds) {
        // Kiểm tra xem hộ gia đình đã có thanh toán phí quản lý tháng 6 chưa
        const existingPayment = paymentsToCreate.find(
          (p) =>
    ...
    console.log(
      "💰 Tạo tình huống đặc biệt: Hộ đã thanh toán tháng 6 nhưng còn nợ tháng 5..."
    );
    ...
    for (const household of specialCaseHouseholds) {
      ...
      const june6Payment = paymentsToCreate.find(
        (p) =>
          p.household.toString() === household._id.toString() &&
          p.fee.toString() === managementFee._id.toString() &&
          p.paymentDate &&
      ...
      if (june6Payment) {
        ...
        const may5Payment = paymentsToCreate.find(
          (p) =>
            p.household.toString() === household._id.toString() &&
            p.fee.toString() === managementFee._id.toString() &&
        ...
        if (!may5Payment) {
          ...
          console.log(
            `✅ Đã tạo khoản nợ tháng 5 cho hộ ${household.apartmentNumber}`
          );
        }
        // Nếu đã có khoản thanh toán tháng 5 nhưng không phải quá hạn, chuyển thành quá hạn
          ...
          console.log(
            `✅ Đã chuyển khoản thanh toán tháng 5 thành quá hạn cho hộ ${household.apartmentNumber}`
          );
        }
        ...
        for (const fee of otherFees) {
          ...
          const existingFeePayment = paymentsToCreate.find(
            (p) =>
              p.household.toString() === household._id.toString() &&
          ...
          if (!existingFeePayment) {
            ...
            console.log(
              `✅ Đã tạo khoản nợ ${fee.name} tháng 5 cho hộ ${household.apartmentNumber}`
            );
          } else if (existingFeePayment.status !== "overdue") {
            existingFeePayment.status = "overdue";
            existingFeePayment.paymentDate = null;
            existingFeePayment.period = new Date(
              new Date().getFullYear(),
              4,
              1
            );
            existingFeePayment.method = null;
            existingFeePayment.note = `${
              fee.name
            } quá hạn - Tháng 5/${new Date().getFullYear()}`;

            console.log(
              `✅ Đã chuyển khoản thanh toán ${fee.name} tháng 5 thành quá hạn cho hộ ${household.apartmentNumber}`
            );
          }
        }
      }
    ...
    if (parkingFees.length > 0) {
      ...
      for (const household of parkingHouseholds) {
        ...
        const numParkingFees = Math.min(
          parkingFees.length,
          Math.floor(Math.random() * 2) + 1
        );
        const selectedParkingFees = parkingFees
          .sort(() => 0.5 - Math.random())
          .slice(0, numParkingFees);
    ...
    const contributionFees = fees.filter(
      (fee) => fee.feeType === "contribution"
    );
    ...
    for (let i = 0; i < paymentsToCreate.length; i += batchSize) {
      ...
      try {
        ...
        console.log(
          `✅ Đã lưu batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(
            paymentsToCreate.length / batchSize
          )}`
        );
      } catch (error) {
        console.log(
          `⚠️ Một số thanh toán trong batch ${
    ...
    const finalHouseholdCount = await Household.countDocuments({
      active: true,
    });
    ...
    console.log(
      `💵 Doanh thu tháng hiện tại: ${currentMonthRevenue.toLocaleString()} VND`
    );
    console.log(
      `📈 Số thanh toán tháng hiện tại: ${currentMonthPayments.length}`
    );

    // Thống kê doanh thu tháng 6 đến ngày 6/6
    const june16Start = new Date(new Date().getFullYear(), 5, 1); // 1/6
    const june16End = new Date(
      new Date().getFullYear(),
      5,
      16,
      23,
    ...
    const june16Payments = await Payment.find({
      paymentDate: { $gte: june16Start, $lte: june16End },
      status: "paid",
    })
      .populate("fee")
      .populate("household");

    const june16Revenue = june16Payments.reduce((sum, p) => sum + p.amount, 0);

    console.log("\n📊 THỐNG KÊ DOANH THU THÁNG 6 (đến ngày 16/6):");
    console.log(`💰 Tổng số thanh toán: ${june16Payments.length}`);
    console.log(`💵 Tổng doanh thu: ${june16Revenue.toLocaleString()} VND`);

    // Thống kê theo loại phí
    const revenueByFeeType = {};
    for (const payment of june16Payments) {
    ...
    for (const [feeType, amount] of Object.entries(revenueByFeeType)) {
      ...
      switch (feeType) {
        case "mandatory":
          feeTypeName = "Phí bắt buộc";
          break;
        case "voluntary":
          feeTypeName = "Phí tự nguyện";
          break;
        case "contribution":
          feeTypeName = "Phí đóng góp";
          break;
        case "parking":
          feeTypeName = "Phí gửi xe";
          break;
        case "utilities":
          feeTypeName = "Phí tiện ích";
          break;
        default:
          feeTypeName = feeType;
      }
    ...
    console.log(
      `💵 Tổng giá trị quá hạn: ${overdueAmount.toLocaleString()} VND`
    );
    ...
    for (const [feeType, data] of Object.entries(overdueByFeeType)) {
      ...
      console.log(
        `${feeTypeName}: ${
          data.count
        } khoản - ${data.amount.toLocaleString()} VND`
      );
    }

    // Đếm số hộ gia đình có khoản quá hạn
    const householdsWithOverdue = [
      ...new Set(overduePayments.map((p) => p.household._id.toString())),
    ];
    console.log(
      `🏠 Số hộ gia đình có khoản quá hạn: ${householdsWithOverdue.length}/${
        allHouseholds.length
      } (${Math.round(
        (householdsWithOverdue.length / allHouseholds.length) * 100
      )}%)`
    );
    ...
    console.log(
      `🏠 Số hộ gia đình có khoản quá hạn nhưng đã thanh toán tháng hiện tại: ${
        householdsWithOverdueAndCurrentPaid.length
      }/${householdsWithOverdue.length} (${Math.round(
        (householdsWithOverdueAndCurrentPaid.length /
          householdsWithOverdue.length) *
          100
      )}%)`
    );
  } catch (error) {
